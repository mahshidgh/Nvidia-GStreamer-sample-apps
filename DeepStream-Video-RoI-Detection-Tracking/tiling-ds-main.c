...
static void source_pad_added(GstElement *element, GstPad *pad, gpointer data)
{
    ...
    //GstElement *decoder = elements_data->decoder;
...
int main(int argc, char *argv[])
{
    ...
    GstElement *osd, *osd_convert, *encoder, *parser, *mux, *sink;

    osd = gst_element_factory_make("nvdsosd", "osd");
    osd_convert = gst_element_factory_make("nvvideoconvert", "osd_convert");
    encoder = gst_element_factory_make("nvv4l2h264enc", "encoder");  // or use "nvv4l2h265enc" for H.265
    parser = gst_element_factory_make("h264parse", "parser");  // or use "h265parse" for H.265
    mux = gst_element_factory_make("qtmux", "mux");  // or use "matroskamux" for MKV
    sink = gst_element_factory_make("filesink", "sink");


    g_object_set (source, "uri", "file://videos/2.mp4",NULL);
    g_object_set(sink, "location", "file1.mp4", NULL);
    // Check if element creation was successful
    if (!source || !convert || !streammux || !nvinfer)
    {
    ...
    {
        nvds_parse_uridecodebin(source, argv[1], "uridecodebin");
   ...
   // g_object_set (decoder, "cudadec-memtype", 0, "num-extra-surfaces", 1, NULL);
    ...
    // Set up the pipeline
    GstElement *pipeline = gst_pipeline_new("my-pipeline");
    gst_bin_add_many(GST_BIN(pipeline), source, convert, streammux, preprocess, nvinfer, tracker, osd, osd_convert, encoder, parser, mux, sink, NULL);

    // Link the elements. Note: you need to handle the 'pad-added' signal when linking dynamic pads.
    // Configure the tracker
    
    g_object_set(G_OBJECT(preprocess), "config-file", "preprocess-tiling-config.txt", NULL);

    // Create link element
    RTSPLinkElementsData elements_data;
   // elements_data.decoder = decoder;
...
//    if (!gst_element_link_many (decoder, queue, convert, NULL) ) {
//         g_printerr ("Elements could not be linked.\n");
//         gst_object_unref (pipeline);
//         return -1;
//     }
    
    if (!gst_element_link_many(streammux, preprocess, nvinfer, NULL))
    {
    ...
    // if (!gst_element_link_many(decoder1, queue1, convert1, NULL))
    // {
    //     g_printerr("Elements could not be linked.\n");
    //     gst_object_unref(pipeline);
    //     return -1;
    ...
    if (!gst_element_link_many(nvinfer, tracker, osd, osd_convert, encoder, parser, mux, sink, NULL))
    {
    ...
    // GstPad *nvinfer_src_pad1 = gst_element_get_static_pad(nvinfer1, "src");
    // gst_pad_add_probe(nvinfer_src_pad1, GST_PAD_PROBE_TYPE_BUFFER, nvinfer_src_pad_buffer_probe1, NULL, NULL);
    // gst_object_unref(nvinfer_src_pad1);
