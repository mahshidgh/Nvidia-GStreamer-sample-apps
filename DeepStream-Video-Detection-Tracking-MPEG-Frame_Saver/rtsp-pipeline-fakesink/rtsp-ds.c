...
int main(int argc, char *argv[])
{
    ...
    if (!source || !depay || !parse || !decoder || !capsfilter || !convert_fps || !videorate || !convert || !streammux || !nvinfer || !tee || !queue1 || !queue2 || !image_convert || !image_encoder || !image_sink)
    {
        g_printerr("One or more elements could not be created. Exiting.\n");
        return -1;
    }
    // Configure the multifilesink
    g_object_set(image_sink, "location", "frames/%d.jpg", "post-messages", TRUE, "async", FALSE, "sync", FALSE, NULL);
    ...
    GstCaps *caps = gst_caps_from_string("video/x-raw,framerate=(fraction)20/1");
    g_object_set(capsfilter, "caps", caps, NULL);
    gst_caps_unref(caps);
    // Set the properties of the elements
    //g_object_set (source, "location", "rtsp://admin:majmun12@cam1-md2.bed.cosmos-lab.org//Streaming/Channels/1", "protocols", 0x00000004, "latency", 2000, "drop-on-latency", TRUE, "timeout", 5000000, NULL);
    g_object_set (decoder, "cudadec-memtype", 0, "num-extra-surfaces", 1, NULL);
    g_object_set (fakesink, "sync", 0, NULL);
    ...
    if (g_str_has_suffix(argv[1], ".yml") || g_str_has_suffix(argv[1], ".yaml"))
    {
        ...
        nvds_parse_file_sink(fakesink, argv[1], "fakesink");
   ...
   // g_object_set (queue, "leaky", 2, "max-size-buffers", 1, NULL);
    ...
    // Set up the pipeline
    GstElement *pipeline = gst_pipeline_new("my-pipeline");
    gst_bin_add_many(GST_BIN(pipeline), source, depay, parse, decoder, convert_fps, videorate, capsfilter, convert, NULL);
    ...
    gst_bin_add_many(GST_BIN(pipeline), tee, queue1, queue2, image_convert, image_encoder, queue3, image_sink, NULL);
   ...
   if (!gst_element_link_many (depay, parse, decoder, convert_fps, videorate, capsfilter, convert, NULL) ) {
    ...
    if (!gst_element_link_many(queue1, fakesink, NULL))
    {
        g_printerr("Failed to link queue1 elements and elements between them. Exiting.\n");
        return -1;
    }

    if (!gst_element_link_many(queue2, image_convert, image_encoder, queue3, image_sink, NULL))
    {
